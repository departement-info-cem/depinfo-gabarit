...
/**
 * Extrait tous les IDs de type 'doc' ou 'autogenerated' (récursif)
 * @param {Array} items
 * @returns {Array<string>}
 */
function extractAllDocIds(items) {
  let ids = [];
  for (const item of items) {
    if (item.type === "doc" && item.id) {
      ids.push(item.id);
    } else if (item.type === "category" && Array.isArray(item.items)) {
      ids = ids.concat(extractAllDocIds(item.items));
    } else if (item.type === "autogenerated" && item.dirName) {
      // Lire tous les fichiers .md du dossier docs/{dirName}
...
function generateSidebarDocs() {
  ...
  const docIds = Array.isArray(sidebars.docs) ? extractAllDocIds(sidebars.docs) : [];
  const content =
    "// Ce fichier est généré automatiquement à partir de sidebars.js\n" +
    "export default " + JSON.stringify(docIds, null, 2) + ";\n";
