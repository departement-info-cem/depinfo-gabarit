...
/**
 * Extrait tous les objets {id, label} pour les entrées de type 'doc', et {id} pour 'autogenerated'.
 * @param {Array} items
...
function extractAllDocEntries(items) {
  ...
  for (const item of items) {
    if (item.type === "doc" && item.id) {
      entries.push({ id: item.id, label: item.label || null });
...
function generateSidebarDocs() {
  ...
  const content =
    ...
    "export default " + JSON.stringify(allEntries, null, 2) + ";\n";
