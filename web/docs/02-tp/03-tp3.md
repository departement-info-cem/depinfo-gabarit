...
```
2. Ajouter une vérification dans `MenuNavigation()` qui activera le bool `possedeCle` lorsque le joueur est dans la salle contenant la clé (`"🗝"`)
```csharp
if (donjon[positionLigne, positionColonne] == "🗝")
    {
        possedeCle = true;
...
On peut lire les données comme un tableau
```csharp
